...
test('requirement audit: static mappings, approved totals and filtered review navigation',async({page})=>{
  ...
  await page.getByLabel('Search field mappings').fill('post weld heat treatment temperature')
  await expect(page.getByRole('region',{name:'Supported field mappings'}).locator('tbody tr')).toHaveCount(3)
  await page.getByLabel('Policy for Post weld heat treatment temperature').first().selectOption('ignored')
  await expect(page.getByLabel('Policy for Post weld heat treatment temperature').first()).toHaveValue('ignored')
  await page.getByLabel('Search field mappings').fill('')
  ...
  await page.getByLabel('Search review fields and values').fill('post weld heat treatment temperature')
  await expect(page.locator('.u-section-heading')).toContainText('Search results')
  await expect(page.locator('.u-review-table').first()).toContainText('Post weld heat treatment temperature')
  await page.getByLabel('Clear field search').click()
  await page.getByRole('button',{name:'Finish review',exact:true}).click()
  await expect(page.getByRole('button',{name:'Confirm Review & Approve',exact:true})).toBeDisabled()
  await expect(page.locator('.u-warning')).toContainText('required fields')
