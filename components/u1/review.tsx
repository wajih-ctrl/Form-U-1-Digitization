...
export function Review({record,onPatch,busy,onApproval}:{record:URecord;onPatch:(body:Record<string,unknown>)=>void;busy:boolean;onApproval:()=>void}) {
  ...
  const needsAttention=(f:UField)=>['Review Required','Not Detected'].includes(f.status)
  ...
  const fields=record.fields.filter(f=>f.section===section&&(filter!=='Needs attention'||needsAttention(f)))
  ...
  const navigable=record.fields.filter(f=>filter!=='Needs attention'||needsAttention(f))
