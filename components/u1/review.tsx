...
export function Review({record,onPatch,busy,onApproval}:{record:URecord;onPatch:(body:Record<string,unknown>)=>void;busy:boolean;onApproval:()=>void}) {
  ...
  const [section,setSection]=useState(initial?.section||sections[0]),[selected,setSelected]=useState(initial?.id),[page,setPage]=useState(initial?.page||1),[zoom,setZoom]=useState(100),[filter,setFilter]=useState('All fields'),[hovered,setHovered]=useState<string>()
  ...
  function select(f:UField,fromDocument=false){revealField.current=fromDocument;setSelected(f.id);setSection(f.section);setPage(f.page);if(filter==='Needs review'&&resolved(f))setFilter('All fields')}
  function fieldAtPoint(clientX:number,clientY:number,sheet:HTMLDivElement){
    ...
    const contains=(f:UField,pad=0)=>x>=f.box[0]-pad&&x<=f.box[0]+f.box[2]+pad&&y>=f.box[1]-pad&&y<=f.box[1]+f.box[3]+pad
    const exact=onPage.filter(f=>contains(f)),candidates=exact.length?exact:onPage.filter(f=>contains(f,.004))
    return candidates.sort((a,b)=>{
      const area=a.box[2]*a.box[3]-b.box[2]*b.box[3]
      if(area)return area
      const distance=(f:UField)=>Math.hypot(x-f.box[0]-f.box[2]/2,y-f.box[1]-f.box[3]/2)
  ...
  return <>
    ...
    <div className="u-review-layout"><section className="u-document-panel"><div className="u-doc-toolbar"><span><FileIcon/>Original document</span><a href={record.pages[page-1]?.url} target="_blank" rel="noreferrer" aria-label="Open full-resolution page"><Maximize2 size={16}/></a></div><div className="u-doc-controls"><div>{record.pages.map((p,i)=><button key={p.id} className={page===i+1?'active':''} onClick={()=>{setHovered(undefined);setPage(i+1)}}>Page {i+1}</button>)}</div><div><button aria-label="Zoom out" disabled={zoom<=75} onClick={()=>setZoom(zoom-25)}><Minus size={15}/></button><span>{zoom}%</span><button aria-label="Zoom in" disabled={zoom>=250} onClick={()=>setZoom(zoom+25)}><Plus size={15}/></button></div></div><div className="u-document-viewport" ref={viewport} tabIndex={0} role="region" aria-label="Scrollable original document"><div className="u-document-sheet interactive" style={{width:`${zoom}%`}} onPointerMove={e=>setHovered(fieldAtPoint(e.clientX,e.clientY,e.currentTarget)?.id)} onPointerLeave={()=>setHovered(undefined)} onClick={e=>{const f=fieldAtPoint(e.clientX,e.clientY,e.currentTarget);if(f)select(f,true)}}><img src={record.pages[page-1]?.url} alt={`Original Form U-1, page ${page}`}/>{hoveredField&&hoveredField.page===page&&hoveredField.id!==active?.id&&<div className="u-source-hover" style={{left:hoveredField.box[0]*100+'%',top:hoveredField.box[1]*100+'%',width:hoveredField.box[2]*100+'%',height:hoveredField.box[3]*100+'%'}}><span>{hoveredField.label}</span></div>}{active&&active.page===page&&<div ref={highlight} className="u-source-highlight" style={{left:active.box[0]*100+'%',top:active.box[1]*100+'%',width:active.box[2]*100+'%',height:active.box[3]*100+'%'}}><span>{active.label}</span></div>}</div></div><div className="u-doc-note"><ScanLine size={14}/>Click a mapped value to select its field · source areas are approximate</div></section>
