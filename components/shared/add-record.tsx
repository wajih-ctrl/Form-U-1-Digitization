  ...
  return <>
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="sm:max-w-lg">
        ...
        <form onSubmit={e => { e.preventDefault(); if (!name.trim() || !summary.trim()) return; addDocument({ name: name.trim(), summary: summary.trim(), stage, type: "Technical note", status: "draft" }); setOpen(false); setName(""); setSummary(""); toast.success("Technical record saved", { description: "Your draft is available in Documents." }) }}>
          <div className="space-y-4 pb-5"><div className="space-y-2"><Label htmlFor="record-name">Record title *</Label><Input id="record-name" required maxLength={160} value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Secondary train test results" /></div><div className="space-y-2"><Label htmlFor="record-summary">Summary *</Label><Textarea id="record-summary" required rows={5} value={summary} onChange={e => setSummary(e.target.value)} placeholder="Describe the findings and next steps…" /></div></div>
