...
- Current Vercel Blob connections use `BLOB_STORE_ID` with a rotating OIDC credential instead of exposing `BLOB_READ_WRITE_TOKEN`. Storage now accepts either authentication mode. The processing route forwards the request's short-lived OIDC credential to its OCR child process, so page/record reads and the completed extraction save use the connected private store.
