...
function currentLayout(record){
  ...
  const fields=record.fields.filter(field=>!/(?:\.boltQuantity|\.boltSize)$/.test(field.id));
  ...
  if(record.fieldPolicies){for(const id of Object.keys(record.fieldPolicies))if(/(?:\.boltQuantity|\.boltSize|\.flangeAttachment|nozzle\.\d+\.attachment)$/.test(id))delete record.fieldPolicies[id]}
  return record;
}
