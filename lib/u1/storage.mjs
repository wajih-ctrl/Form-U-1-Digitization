...
export function createStorage({env=process.env,client=blob,root=dataDir}={}){
  ...
  function remote(){
    ...
    if(env.VERCEL)throw new Error('Production storage is not configured. Connect a private Vercel Blob store to this project, enable BLOB_READ_WRITE_TOKEN for Production, and redeploy.');
    return false;
  }
  ...
  const options=()=>({token:env.BLOB_READ_WRITE_TOKEN});
