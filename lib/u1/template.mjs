...
export function templateFields() {
  ...
  const shellCols=[['course','Course',37,23],['diameter','Diameter',61,79],['length','Length',141,76],['material','Material',218,114],['thickness','Nominal thickness',334,65],['corrosion','Corrosion allowance',401,62],['longType','Longitudinal joint type',465,27],['longExam','Longitudinal RT / examination',494,73],['longEfficiency','Longitudinal efficiency',570,25],['circType','Circumferential joint type',598,25],['circExam','Circumferential RT / examination',625,72],['circEfficiency','Circumferential efficiency',700,42],['heatTemp','Post weld heat treatment temperature',746,52],['heatTime','Post weld heat treatment time',801,52]];
  ...
  const pressure = (prefix,section,page,dy) => {
    ...
    line(prefix+'.test','Test type and pressure',section,page,219,768+dy,130,18);
  ...
  grid('nozzle','Nozzles / openings',2,'Nozzles / openings',[[607,16],[625,16],[642,16],[660,16],[677,16]],[['purpose','Purpose',33,109],['number','Number',145,33],['size','Diameter / size',182,67],['type','Type',252,56],['material','Nozzle material',312,76],['flangeMaterial','Flange material',392,76],['thickness','Thickness',472,40],['corrosion','Corrosion allowance',516,39],['reinforcement','Reinforcement type',559,68],['attachmentDetails','Nozzle / flange attachment details',627,132],['location','Location',763,90]]);
  ...
  grid('remarks','Remarks',2,'Section 22 remarks',[[812,20],[838,20],[864,20]],[['text','Remark',44,810]]);
  ...
  line('cert.fieldExceptions','Field assembly inspection · exceptions',c,3,39,557,191,6);
  ...
  const critical=id=>/^(manufacturer|serial|nationalBoard|year|shell\.courses|shell\.\d+\.(material|thickness|corrosion|longExam|longEfficiency|circExam|circEfficiency|heatTemp|heatTime)|head\.material|head\.\d+\.(thickness|corrosion|jointType|efficiency)|design\.(mawp|temperature|mdmt|impact|test|testPressure|testType)|tubesheet\.\d+\.(material|diameter|thickness)|tube\.\d+\.(material|od|thickness|number)|nozzle\.\d+\.(purpose|number|size|material|thickness|corrosion|reinforcement|attachment)|cert\.(manufacturer|shopDate|inspector|commission))/.test(id);
  const required=id=>/^(year|shell\.courses|design\.(mawp|temperature|mdmt|test))$/.test(id);
  return fields.map(field=>({...field,importance:critical(field.id)?'Key engineering':'Standard',requirement:required(field.id)?'required':'optional'}));
}
