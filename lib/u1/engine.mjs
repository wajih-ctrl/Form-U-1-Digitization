...
export const dataDir = path.join(process.cwd(), '.u1-data');
export function safeId(id) { if (!/^[a-zA-Z0-9-]{1,80}$/.test(id)) throw new Error('Invalid document identifier'); return id; }
...
export async function saveRecord(record) {
  ...
  // Atomic rename prevents interrupted writes from corrupting the record.
  const {rename} = await import('node:fs/promises');
  const dest = recordPath(record.id), temp = `${dest}.${randomUUID()}.tmp`;
  await writeFile(temp, JSON.stringify(record)); await rename(temp,dest);
}
export async function getRecord(id) { return JSON.parse(await readFile(recordPath(id),'utf8')); }
export async function savePage(buffer, name) {
  ...
  await writeFile(pagePath(id),result.data);
...
export async function extract(pages,onProgress=()=>{}) {
  ...
  try {
    ...
    for(let i=0;i<3;i++) {
      ...
      const buffer=await readFile(pagePath(pages[i].id));
