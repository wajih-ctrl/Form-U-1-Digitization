...
import { detectChangedInk,changedInkCandidate } from './overlay-detection.mjs';
...
async function cellImage(buffer,field,width,height) {
  ...
  const glyphs=components.filter(c=>c.height>=ch*.38&&c.count>=4).sort((a,b)=>a.minX-b.minX);
  // A decimal is a small baseline component between full-height glyphs.
  // Restrict it to the text's occupied span so table specks cannot add a dot.
  const dots=components.filter(c=>c.minY>ch*.42&&c.height<=ch*.3&&c.width<=ch*.25&&c.count>=2&&glyphs.some(g=>g.maxX<c.minX)&&glyphs.some(g=>g.minX>c.maxX));
  const dot=dots.sort((a,b)=>Math.abs(a.minY-ch*.65)-Math.abs(b.minY-ch*.65)||b.count-a.count)[0];
...
async function focusedImage(buffer,field,width,height,dx,dy,dw,dh) {
  const [x,y,w,h]=field.box;
  const rect={left:Math.max(0,Math.floor((x+w*dx)*width)),top:Math.max(0,Math.floor((y+h*dy)*height)),width:Math.ceil(w*dw*width),height:Math.ceil(h*dh*height)};
  return sharp(buffer).extract(rect).greyscale().normalize().resize({width:rect.width*4}).extend({left:20,right:20,top:20,bottom:20,background:'#fff'}).png().toBuffer();
}
const compact=value=>value.replace(/[^A-Z0-9]/gi,'').toUpperCase();
export function technicalValue(value,field,leadingMinus=false) {
  let normalized=value.replace(/[“”]/g,'"').replace(/[‘’`]/g,"'").replace(/\s+/g,' ').trim();
  let repair=false;
  // A small inch mark can look like a star in any field, including free text.
  // Require a preceding digit and a unit boundary so multiplication remains intact.
  const quoted=normalized.replace(/(\d)\*(?=\s|$|[x×,;)\-])/g,'$1"');
  repair ||= quoted!==normalized;normalized=quoted;
  normalized=normalized.replace(/^[^\p{L}\p{N}-]+|[^\p{L}\p{N})%'"\u00b0]+$/gu,'');
  if((/^remarks\.\d+\.text$/.test(field.id)||field.id==='notes')&&/\.$/.test(value.trim())&&!normalized.endsWith('.'))normalized+='.';
  if(field.id==='asme'){
    const section=normalized.replace(/\bSection\s+V(?:I|l){2,3}\b/i,'Section VIII');
    repair ||= section!==normalized;normalized=section;
  }
  if(field.id==='jacket.details'){
    const dimension=normalized.replace(/^[-–]\s+(?=\d+\s*\/\s*\d+)/,'');
    repair ||= dimension!==normalized;normalized=dimension;
  }
  if(/\.bolting$/.test(field.id)){
    const countAndSize=normalized.replace(/^(\d+)\s*[.,]\s*(?=\d+(?:\/\d+)?["'])/,'$1 - ');
    repair ||= countAndSize!==normalized;normalized=countAndSize;
  }
  if(field.table&&/^N[\\/|]?A$/i.test(normalized)){repair ||= normalized!=='N/A';normalized='N/A'}
  if(field.id.endsWith('.mdmt')&&leadingMinus&&/^\d/.test(normalized))normalized='-'+normalized;
  if(/material/i.test(field.label)){
    normalized=normalized.toUpperCase().replace(/^[L|]\s+(?=SA\d)/,'').replace(/^5A(?=\d)/,'SA').replace(/^S[AIL](?=\d)/,'SA').replace(/^SAL(?=\d)/,'SA');
    if(/\.washerMaterial$/.test(field.id)){
      const washer=normalized.replace(/^(?:8|B|5|S|\$){1,2}316$/,'SS316');
      repair ||= washer!==normalized;normalized=washer;
    }
    const separator=normalized.replace(/^(SA\d{3})[. ](?=[B8][78](?:M|N)?\b)/,'$1-');
    repair ||= separator!==normalized;normalized=separator;
    // ASME bolting grade letters occupy a fixed position after the spec.
    const grade=normalized.replace(/^(SA(?:193|320|354)[ -])8(?=[78](?:M|N)?\b)/,'$1B');
    repair ||= grade!==normalized;normalized=grade;
    if(/\.boltMaterial$/.test(field.id)){
      const suffix=normalized.replace(/^(SA193[ -]B[78])N\b/,'$1M');
      repair ||= suffix!==normalized;normalized=suffix;
    }
    if(/^tube\.\d+\.material$/.test(field.id)){
      const suffix=normalized.replace(/^(SA\d{3})L\b/,'$1');
      repair ||= suffix!==normalized;normalized=suffix;
    }
  }
  if(/efficiency/i.test(field.label)&&!/^N\/?A$/i.test(normalized)&&/^[\dSOBIl]+$/i.test(normalized)){
    const numeric=normalized.replace(/[sS]/g,'5').replace(/[oO]/g,'0').replace(/[bB]/g,'8').replace(/[Il]/g,'1');
    repair ||= numeric!==normalized;normalized=numeric;
  }
  if(field.id.endsWith('.impact')){
    const impact=normalized.replace(/\bCE\b/gi,'').replace(/^(?:is|iS)\s+(?=Yes\b|No\b)/,'').replace(/\s{2,}/g,' ').replace(/^[,;\s-]+|[,;\s-]+$/g,'');
    repair ||= impact!==normalized;normalized=impact;
  }
  if(field.id.endsWith('.attachmentDetails')){
    const bracket=normalized.replace(/[|]/g,'').replace(/\)\s*\(\s+(?=[A-Za-z])/g,') ').replace(/^\(([A-Za-z]{1,3})\s+(?=[A-Za-z])/,'($1) ').replace(/^([A-Za-z]{1,3})[)}]?\s+(?=[A-Za-z])/,'($1) ').replace(/(?<!\()\b([A-Za-z]{1,3})\)/g,'($1)').replace(/(?<!\()\b([A-Za-z]{1,3})\}/g,'($1)');
    repair ||= bracket!==normalized;normalized=bracket;
  }
  if(field.id.endsWith('.reinforcement')&&/^PA[CQO]$/i.test(normalized)){
    const type=normalized.replace(/^PA[CQO]$/i,'PAD');
    repair ||= type!==normalized;normalized=type;
  }
  if(/(?:joint|attachment)/i.test(field.label)||/\.(?:longExam|circExam)$/.test(field.id)){
    const pen=normalized.replace(/\bull\s+pen\b/i,'Full pen');
    repair ||= pen!==normalized;normalized=pen;
  }
  ...
  return {value:normalized,repair};
}
...
function confidenceReason(value,confidence,field,disagreed,repaired) {
  if(!value)return 'No value was detected in the mapped source area.';
  if(repaired)return 'Ambiguous OCR character repaired from field context; verify against the document.';
...
export async function extract(pages,onProgress=()=>{},signal) {
  ...
  try {
    ...
    const changedInkPages=await Promise.all(ordered.map((scan,index)=>detectChangedInk(scan.buffer,index+1)));
    const referenceAligned=changedInkPages.filter(Boolean).length>=2;
    for(let i=0;i<3;i++) {
      const {buffer,meta,data}=ordered[i],words=wordsOf(data);
      const changedInk=referenceAligned?changedInkPages[i]:null;
      let changedWords=[];
      if(changedInk){
        onProgress({stage:'reading',message:`Page ${i+1}: checking changed text against the supported reference layout`,page:i+1});
        const changedReading=await worker.recognize(changedInk.image,{}, {blocks:true,text:true});
        changedWords=wordsOf(changedReading.data);
      }
      raw.push(data.text);
      for(const field of fields.filter(f=>f.page===i+1)) {
        ...
        if(!cell.image) {value='';confidence=0;}
          ...
          if(same)confidence=Math.max(confidence,fullConfidence);
            ...
            // The page pass sees context and often retains tiny feet/inch
            // marks that vanish when the tight cell is binarized.
            const dimension=/\b(?:length|diameter|size|thickness|radius|allowance|OD|ID)\b/i.test(field.label);
            const fullerUnits=dimension&&cellKey===fullKey&&/["']/.test(fullValue)&&((fullValue.match(/["']/g)||[]).length>(value.match(/["']/g)||[]).length);
            const fullerDecimal=dimension&&cellKey===fullKey&&/\d\.\d/.test(fullValue)&&!/\d\.\d/.test(value);
            const missingFirstCharacter=cellKey.length>=4&&fullKey.length===cellKey.length+1&&fullKey.endsWith(cellKey);
            const missingRemarkPrefix=/^remarks\.\d+\.text$/.test(field.id)&&cellKey.length>=5&&fullKey.endsWith(cellKey)&&fullKey.length>cellKey.length;
            const betterMaterial=/material/i.test(field.label)&&valueFit(fullValue,field)>=valueFit(value,field)+5;
            if((fullerUnits||fullerDecimal)&&fullConfidence>=confidence-20){value=fullValue;confidence=Math.max(confidence,fullConfidence)}
            else if(missingRemarkPrefix&&fullConfidence>=confidence-25){value=fullValue;confidence=Math.max(confidence,fullConfidence)}
            else if(betterMaterial&&fullConfidence>=confidence-25){value=fullValue;confidence=fullConfidence}
            else if(missingFirstCharacter&&fullConfidence>=confidence-6){value=fullValue;confidence=fullConfidence}
            else if(!cellIsContained&&confidence<75&&fullConfidence>confidence+12&&!value.startsWith('-')){value=fullValue;confidence=fullConfidence}
            if(fullValue)confidence=Math.min(confidence,79);
          }
          cell.disagreed=disagreed;
          if((!value||!/\d/.test(value))&&/\.(?:course|longType|circType|number|size)$/.test(field.id)&&!/\b(?:number of tubes|bolting|washer)\b/i.test(field.label)){
            await worker.setParameters({tessedit_pageseg_mode:PSM.SINGLE_CHAR,tessedit_char_whitelist:'0123456789'});
            const single=await worker.recognize(cell.alternate??cell.image,{}, {text:true});
            const digit=single.data.text.trim().match(/^\d$/)?.[0];
            if(digit){value=digit;confidence=Math.round(single.data.confidence)}
            await worker.setParameters({tessedit_pageseg_mode:PSM.SINGLE_LINE,tessedit_char_whitelist:''});
          }
        }
        if(!cell.redacted&&(!value||!/\d/.test(value))&&/\.(?:circType|longType|size)$/.test(field.id)){
          for(const crop of [[.15,.18,.7,.65],[.25,.2,.5,.6]]){
            const image=await focusedImage(buffer,field,meta.width,meta.height,...crop);
            await worker.setParameters({tessedit_pageseg_mode:PSM.SINGLE_LINE,tessedit_char_whitelist:''});
            const reading=await worker.recognize(image,{}, {text:true});
            const candidate=reading.data.text.trim();
            if(/^\d+(?:\.\d+)?["']?$/.test(candidate)&&reading.data.confidence>=55){value=candidate;confidence=Math.round(reading.data.confidence);break}
          }
        }
        if(!cell.redacted&&/\.length$/.test(field.id)&&/^\d{2,}["']?$/.test(value)){
          const image=await focusedImage(buffer,field,meta.width,meta.height,0,0,1,.65);
          await worker.setParameters({tessedit_pageseg_mode:PSM.SINGLE_LINE,tessedit_char_whitelist:''});
          const reading=await worker.recognize(image,{}, {text:true}),candidate=reading.data.text.trim();
          if(compact(candidate)===compact(value)&&/[.'"]/.test(candidate)&&reading.data.confidence>=65){value=candidate;confidence=Math.round(reading.data.confidence)}
        }
        if(!cell.redacted&&/^(?:bodyFlange|headFlange|innerFlange|innerHeadFlange)\.\d+\.type$/.test(field.id)){
          if(/^s[o0]$/i.test(value))value='SO';
          else if(/^[o0]$/.test(value)){
            const image=await focusedImage(buffer,field,meta.width,meta.height,0,0,1,1);
            await worker.setParameters({tessedit_pageseg_mode:PSM.SINGLE_LINE,tessedit_char_whitelist:''});
            const reading=await worker.recognize(image,{}, {text:true});
            if(/^so$/i.test(reading.data.text.trim())){value='SO';confidence=Math.min(79,Math.round(reading.data.confidence))}
          }
        }
        if(!cell.redacted&&/^nozzle\.\d+\.purpose$/.test(field.id)&&confidence<60){
          const image=await focusedImage(buffer,field,meta.width,meta.height,0,-.2,1,1.15);
          await worker.setParameters({tessedit_pageseg_mode:PSM.SINGLE_LINE,tessedit_char_whitelist:''});
          const reading=await worker.recognize(image,{}, {text:true}),candidate=reading.data.text.trim();
          if(/^[A-Z]{2,}\/[A-Z]{2,}$/i.test(candidate)&&reading.data.confidence>confidence+10){value=candidate;confidence=Math.round(reading.data.confidence)}
        }
        if(!cell.redacted&&/^remarks\.\d+\.text$/.test(field.id)&&confidence<90){
          const image=await focusedImage(buffer,field,meta.width,meta.height,0,0,1,1);
          await worker.setParameters({tessedit_pageseg_mode:PSM.SINGLE_LINE,tessedit_char_whitelist:''});
          const reading=await worker.recognize(image,{}, {text:true});
          if(reading.data.text.trim().length>=value.length*.8&&reading.data.confidence>confidence+5){value=reading.data.text.trim();confidence=Math.round(reading.data.confidence)}
        }
        if(field.id.endsWith('.attachmentDetails')&&cell.image){
          const [bx,by,,bh]=field.box,parts=[];
          for(const [offset,width] of [[2,52],[70,58]]){
            const crop=await cellImage(buffer,{...field,box:[bx+offset/922,by,width/922,bh]},meta.width,meta.height);
            if(!crop.image){parts.push({value:'',confidence:0});continue}
            const reading=await worker.recognize(crop.image,{}, {text:true});
            parts.push({value:reading.data.text.trim().replace(/\s+/g,' '),confidence:Math.round(reading.data.confidence)});
          }
          if(parts.some(part=>part.value)){
            value=parts.map(part=>part.value).filter(Boolean).join(' ');
            confidence=Math.min(...parts.filter(part=>part.value).map(part=>part.confidence));
          }
          if(confidence<70){
            const image=await focusedImage(buffer,field,meta.width,meta.height,0,-.2,1,1.15);
            await worker.setParameters({tessedit_pageseg_mode:PSM.SINGLE_LINE,tessedit_char_whitelist:''});
            const reading=await worker.recognize(image,{}, {text:true});
            if(/\b(?:full\s*pen|threaded|butt|welded)\b/i.test(reading.data.text)&&reading.data.confidence>confidence+10){value=reading.data.text.trim();confidence=Math.round(reading.data.confidence)}
          }
        }
        const normalized=technicalValue(value,field,cell.leadingMinus);
        ...
        if(field.id.endsWith('.impact'))value=value.replace(/^.*?\b(?=Yes\b|No\b)/i,'').replace(/\bCE\b/gi,'').trim();
        let lowConfidenceReason=ambiguous&&value?'Name and address share one source line; verify the split.':confidenceReason(value,confidence,field,cell.disagreed||false,normalized.repair);
        const changed=changedInk?.inkIn(field.box);
        if(changed&&changed.count>=22&&changed.ratio>=.008){
          const alternate=changedInkCandidate(changedWords,field,changedInk.width,changedInk.height);
          if(alternate){
            const candidate=technicalValue(alternate.value,field).value;
            const numerical=/efficiency|pressure|temperature|diameter|length|thickness|size|radius|allowance|\bOD\b|\bID\b/i.test(field.label);
            const textual=/\b(?:type|attachment|purpose|location|remarks?|notes?)\b/i.test(field.label);
            const plausible=/[a-zA-Z0-9]/.test(candidate)&&(!numerical||/\d|N\/?A/i.test(candidate))&&(!textual||/[a-zA-Z]{2}|N\/?A/i.test(candidate))&&(!/material/i.test(field.label)||/^(?:SA|SB|ASTM|A)\s*\d|^N\/?A$|^NONE$|^PAD$/i.test(candidate));
            const candidateLosesUnit=/["']/.test(value)&&!/["']/.test(candidate)&&/\b(?:length|diameter|size|thickness|radius)\b/i.test(field.label);
            if(plausible&&!candidateLosesUnit&&(alternate.confidence>=70||numerical&&alternate.confidence>=55)){
              value=candidate;
              confidence=Math.min(79,alternate.confidence);
            }
          }
          lowConfidenceReason='Changed text was detected over the reference form. Verify the new value against the document; an underlying older value may still be visible.';
        }
