...
export function technicalValue(value,field,leadingMinus=false) {
  ...
  if(field.id.endsWith('.impact')){
    const impact=normalized.replace(/\bCE\b/gi,'').replace(/\s{2,}/g,' ').replace(/^[,;\s-]+|[,;\s-]+$/g,'');
    repair ||= impact!==normalized;normalized=impact;
  }
  if(field.id.endsWith('.attachmentDetails')){
    const bracket=normalized.replace(/[|]/g,'').replace(/^([A-Za-z]{1,3})[)}]?\s+(?=[A-Za-z])/,'($1) ').replace(/(?<!\()\b([A-Za-z]{1,3})\)/g,'($1)').replace(/(?<!\()\b([A-Za-z]{1,3})\}/g,'($1)');
      ...
      for(const field of fields.filter(f=>f.page===i+1)) {
        ...
        if(!cell.image) {value='';confidence=0;}
          ...
          if(same)confidence=Math.max(confidence,fullConfidence);
            ...
            if(fullerUnits&&fullConfidence>=confidence-15){value=fullValue;confidence=Math.max(confidence,fullConfidence)}
        ...
        if(field.id.endsWith('.attachmentDetails')&&cell.image){
          ...
          for(const [offset,width] of [[7,44],[62,47]]){
          ...
          if(parts.every(part=>part.value)){
        ...
        const lowConfidenceReason=ambiguous&&value?'Name and address share one source line; verify the split.':confidenceReason(value,confidence,field,cell.disagreed||false,normalized.repair);
