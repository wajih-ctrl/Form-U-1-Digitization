...
export function technicalValue(value,field,leadingMinus=false) {
  ...
  let repair=false;
  // In dimension cells OCR commonly substitutes an asterisk for an inch mark.
  // Preserve an actual quote; only repair a star immediately following a digit.
  if(/(?:diameter|length|thickness|size|radius|allowance|hub|OD|ID|bolting)/i.test(field.label)){
  ...
  if(/^remarks\.\d+\.text$/.test(field.id)&&/\.$/.test(value.trim())&&!normalized.endsWith('.'))normalized+='.';
